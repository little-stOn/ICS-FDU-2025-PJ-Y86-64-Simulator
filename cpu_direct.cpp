...
void L1Cache::load_line(uint64_t block_addr, uint64_t index)
{
    ...
    // Load from memory (cache miss penalty)
...
void L1Cache::write_back_line(uint64_t index)
{
    ...
    // Write to memory (write-back penalty)
    total_memory_time += MEMORY_ACCESS_TIME;
    memory_writes++;
...
uint8_t L1Cache::get_byte(uint64_t addr, bool &error)
{
    ...
    // Cache access takes CACHE_ACCESS_TIME cycles
    ...
    {
        misses++;
        // Cache miss: load from memory (handled in load_line)
...
void L1Cache::print_report()
{
    ...
    cerr << endl;
    // cerr << "Cache Access Time (per access): " << CACHE_ACCESS_TIME << " cycle(s)" << endl;
    // cerr << "Memory Access Time (per access): " << MEMORY_ACCESS_TIME << " cycle(s)" << endl;
    // cerr << "Memory Reads (cache misses): " << memory_reads << endl;
    // cerr << "Memory Writes (write-backs): " << memory_writes << endl;
    // cerr << endl;
